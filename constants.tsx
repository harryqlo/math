...
export const THEORY_DATA: TheorySet = {
  cuadratica: {
    ...
    paragraphs: [
      'Una función cuadrática es un polinomio que tiene un término con $\\mathbf{x^2}$. Su "receta" o fórmula general es $\\mathbf{f(x) = ax^2 + bx + c}$. En esta receta, $\\mathbf{a}$, $\\mathbf{b}$ y $\\mathbf{c}$ son números fijos que llamamos "coeficientes". El coeficiente $\\mathbf{a}$ es el que multiplica a $\\mathbf{x^2}$, $\\mathbf{b}$ multiplica a $\\mathbf{x}$, y $\\mathbf{c}$ es el término independiente. Hay una regla de oro: el coeficiente $\\mathbf{a}$ \\textbf{nunca puede ser cero}. Si lo fuera, el término $\\mathbf{x^2}$ se anularía y nos quedaría una simple función lineal (una línea recta), no una parábola.',
      'Su representación gráfica es una curva suave y simétrica llamada \\textbf{parábola}. Imagina la trayectoria que sigue una pelota al ser lanzada al aire; esa curva describe una parábola. Comprender cada uno de sus componentes nos permite predecir y dibujar su forma sin necesidad de calcular docenas de puntos.'
  ...
  logaritmica: {
    ...
    paragraphs: [
      ...
      'Son indispensables para manejar escalas muy grandes (como en terremotos o en el pH) y para resolver ecuaciones donde la incógnita aparece en el exponente.'
    ...
    properties: [
      ...
      { term: 'Rango', description: 'A diferencia del dominio, el \\textbf{Rango} de una función logarítmica es el conjunto de todos los números reales, $\\mathbf{(-\\infty, \\infty)}$. Esto se debe a que el logaritmo es la inversa de la exponencial, y el dominio de la exponencial (todos los reales) se convierte en el rango del logaritmo.' },
